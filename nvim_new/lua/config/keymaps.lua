...
-- Alt-Backspace: delete previous "word" per [0-9A-Za-z_] with your exact whitespace rules,
-- and make it a clean, single undo step.
-- Behavior:
--   some text|   -> some  |
--   some tex|t   -> some |t
--   some text |  -> some |
...
--   this is a|   -> this is |
--
-- Registers are untouched. Undo: a single 'u' restores the deletion.
...
local function alt_bs_delete_prev_word()
...
local opts = { silent = true, desc = "Alt-Backspace: delete previous word (undo-friendly)" }
vim.keymap.set("i", "<A-BS>", alt_bs_delete_prev_word, opts)
